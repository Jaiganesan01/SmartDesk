...

{
  ...
  "runbook_action":      "One of: flush_dns_vpn, okta_password_reset, restart_service, reprovision_email, none"
}
...
def redact_pii(text: str) -> str:
    """Masks credit cards, API keys, passwords, and sensitive credentials."""
    if not text:
        return text
    text = re.sub(r'\b(?:\d[ -]*?){13,16}\b', '[REDACTED_PAYMENT_CARD]', text)
    text = re.sub(r'(?i)(password\s*(?:is|:)?\s*)([A-Za-z0-9!@#$%^&*]+)', r'\1[REDACTED_PASSWORD]', text)
    text = re.sub(r'\b(?:sk-[A-Za-z0-9]{20,}|ghp_[A-Za-z0-9]{30,}|[0-9a-fA-F]{32,})\b', '[REDACTED_API_TOKEN]', text)
    return text
...
def get_mock_extraction(transcript: str) -> dict:
    ...
    # Determine autonomous runbook capabilities
    can_auto_resolve = False
    runbook_action = "none"
    category = matched.get("category", "")

    if "vpn" in text or "network" in text or "dns" in text:
        can_auto_resolve = True
        runbook_action = "flush_dns_vpn"
    elif "password" in text or "lockout" in text or "okta" in text or "login" in text:
        can_auto_resolve = True
        runbook_action = "okta_password_reset"
    elif "crash" in text or "service" in text or "daemon" in text:
        can_auto_resolve = True
        runbook_action = "restart_service"
    elif "outlook" in text or "email" in text:
        can_auto_resolve = True
        runbook_action = "reprovision_email"
